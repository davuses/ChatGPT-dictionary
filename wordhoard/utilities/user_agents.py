...
def _unpickle_user_agents():
    ...
    """
    try:
        _common_user_agents = os.path.join(
            BASE_DIR, "files/common_user_agents.pkl"
        )
        ...
        logger.error(
            "The common_user_agents.pkl file was not found. Aborting operation."
        )
        logger.error("".join(traceback.format_tb(error.__traceback__)))
        sys.exit(1)
    except OSError as error:
        logger.error(
            "An OS error occurred when trying to open the file common_user_agents.pkl"
        )
        logger.error("".join(traceback.format_tb(error.__traceback__)))
        sys.exit(1)
...
def get_specific_user_agent(requested_key: str) -> Union[str, None]:
    ...
    if bool([v for k, v in user_agent_keys.items() if k == requested_key]):
        key = "".join(
            [v for k, v in user_agent_keys.items() if k == requested_key]
        )
        user_agents_list = _unpickle_user_agents()
        ...
        logger.error(
            "The requested user agent was not found in the list of available agents."
        )
