...
class Query:
    ...
    def _handle_http_status_codes(self, status_code: int):
        ...
        if status_code == 404:
            ...
            colorized_text(
                text="An HTTP 500 Internal Server Error has occurred."
            ...
            colorized_text(
                text="A 503 Service Unavailable status code has been detected."
                "\nPlease review the WordHoard logs for additional information.",
                color="red",
            )
            logger.info("-" * 80)
            ...
            colorized_text(
                text="An HTTP 504 Gateway Timeout Error status code has been detected."
                "\nPlease review the WordHoard logs for additional information.",
                color="red",
            )
            logger.info("-" * 80)
            ...
            colorized_text(
                text="A 521 status code has been detected. This code is often used by CloudFlare."
                "\nPlease review the WordHoard logs for additional information.",
                color="red",
            )
            logger.info("-" * 80)
    ...
    def _handle_exceptions(self, error):
        ...
        if isinstance(error, requests.HTTPError):
            colorized_text(
                text="A HTTPError has occurred."
                "\nPlease review the WordHoard logs for additional information.",
                color="red",
            )
            logger.error(
                f"A HTTPError has occurred when requesting {self._url_to_scrape}"
            )
            logger.error("".join(traceback.format_tb(error.__traceback__)))
            sys.exit(1)
        elif isinstance(error, requests.URLRequired):
            logger.error(
                f"A URLRequired has occurred when requesting {self._url_to_scrape}"
            )
            logger.error("".join(traceback.format_tb(error.__traceback__)))
            sys.exit(1)
        elif isinstance(error, requests.exceptions.ProxyError):
            colorized_text(
                text="A unknown type of Proxy Error has occurred."
                "\nPlease verify that your proxies are working.",
                color="red",
            )
            logger.error(
                f"A ProxyError has occurred when requesting {self._url_to_scrape}"
            )
            logger.error("".join(traceback.format_tb(error.__traceback__)))
            sys.exit(1)
        elif isinstance(error, MaxRetryError):
            colorized_text(
                text="The max number of connection retries was exceeded."
                "\nPlease review the WordHoard logs for additional information.",
                color="red",
            )
            logger.error(
                f"A MaxRetryError has occurred when requesting {self._url_to_scrape}"
            )
            logger.error("".join(traceback.format_tb(error.__traceback__)))
            sys.exit(1)
        elif isinstance(error, requests.ConnectionError):
            colorized_text(
                text="A ConnectionError has occurred."
                "\nPlease review the WordHoard logs for additional information.",
                color="red",
            )
            logger.error(
                f"A ConnectionError has occurred when requesting {self._url_to_scrape}"
            )
            logger.error("".join(traceback.format_tb(error.__traceback__)))
            sys.exit(1)
        elif isinstance(error, requests.Timeout):
            colorized_text(
                text="A connection timeout has occurred."
                "\nPlease review the WordHoard logs for additional information.",
                color="red",
            )
            logger.error(
                f"A Timeout has occurred when requesting {self._url_to_scrape}"
            )
            logger.error("".join(traceback.format_tb(error.__traceback__)))
            sys.exit(1)
        elif isinstance(error, requests.RequestException):
            colorized_text(
                text="A RequestException has occurred."
                "\nPlease review the WordHoard logs for additional information.",
                color="red",
            )
            logger.error(
                f"A RequestException has occurred when requesting {self._url_to_scrape}"
            )
            logger.error("".join(traceback.format_tb(error.__traceback__)))
            sys.exit(1)
        ...
        try:
            ...
            cloudflare_protected = bool(
                [
                    ...
                    if key == "Server" and value == "cloudflare"
                ]
            )
            ...
            if response.status_code in {404, 500, 503, 504, 521}:
                ...
                if cloudflare_protected is True:
                    logger.info("-" * 80)
                    logger.info(
                        "The requested URL is protected by Cloudflare's DDoS mitigation service."
                    )
                    logger.info(f"Requested URL: {self._url_to_scrape}")
                    logger.info("-" * 80)
                    ...
                    logger.info(
                        "HTTP 403 is an HTTP status code meaning access to the requested "
                ...
                if response.status_code != 200:
                    colorized_text(
                        text=f"The Status Code: {response.status_code} has been detected."
                        "\nPlease review the WordHoard logs for additional information.",
                        color="red",
                    )
                    logger.error(
