...
def insert_word_cache_antonyms(
    word: str, pos_category: str, antonyms: Set[str]
) -> None:
    """
    ...
    if word not in temporary_dict_antonyms:
        ...
        deduplicated_values = set(antonyms) - set(
            temporary_dict_antonyms[word][pos_category]
        )
...
def insert_word_cache_definition(
    word: str, pos_category: str, definitions: Set[str]
) -> None:
    """
    ...
    if word not in temporary_dict_definition:
        ...
        word in temporary_dict_definition
        and pos_category in temporary_dict_definition[word]
    ):
        # Deduplicate and update the existing definitions list
        deduplicated_values = set(definitions) - set(
            temporary_dict_definition[word][pos_category]
        )
        temporary_dict_definition[word][pos_category] += list(
            deduplicated_values
        )
    else:
        # Initialize the list for the part of speech category
...
def insert_word_cache_hypernyms(word: str, values: List[str]) -> None:
    ...
    if word in temporary_dict_hypernyms:
        deduplicated_values = set(values) - set(
            temporary_dict_hypernyms.get(word)
        )
...
def insert_word_cache_hyponyms(word: str, values: List[str]) -> None:
    ...
    if word in temporary_dict_hyponyms:
        deduplicated_values = set(values) - set(
            temporary_dict_hyponyms.get(word)
        )
